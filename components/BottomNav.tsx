...
export default function BottomNav({ urgentCount = 0 }: Props) {
  ...
  return (
    <nav className="flex border-t border-black/8 flex-shrink-0 bg-white">
      <Link href="/" className={`flex-1 py-3 px-1 text-[13px] font-semibold text-center flex flex-col items-center gap-1.5 ${active('/')}`} style={{minHeight:'60px'}}>
      ...
      <Link href="/caderno" className={`flex-1 py-3 px-1 text-[13px] font-semibold text-center flex flex-col items-center gap-1.5 relative ${active('/caderno')}`} style={{minHeight:'60px'}}>
        <svg width="24" height="24" viewBox="0 0 18 18" fill="none">
          <rect x="2" y="2" width="14" height="14" rx="2.5" stroke="currentColor" strokeWidth="1.4" />
        ...
        Caderno
        {urgentCount > 0 && (
          <span className="absolute top-2 right-4 bg-red-500 text-white rounded-full text-[11px] px-1.5 py-px font-bold leading-none">
            {urgentCount}
          </span>
        )}
      </Link>

      <Link href="/analytics" className={`flex-1 py-3 px-1 text-[13px] font-semibold text-center flex flex-col items-center gap-1.5 ${active('/analytics')}`} style={{minHeight:'60px'}}>
        <svg width="24" height="24" viewBox="0 0 18 18" fill="none">
